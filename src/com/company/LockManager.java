...
public class LockManager {
    ...
    private HashMap<Transaction,ArrayList<Pair<DataItem,String>>> newlocktable = new HashMap<>();
    ...
    public void LS(Transaction t, DataItem i){
        ...
        if (newlocktable.get(t).contains(Pair.with(i, "S")) || newlocktable.get(t).contains(Pair.with(i, "X"))) {
            ...
            if(i.getWaitqueue().isEmpty()) {
                t.getBlockedItens().add(Pair.with(i,"S"));
                newlocktable.put(t,t.getBlockedItens()); // Adiciona na key(transaction) t, o par (i,S)
                i.setLocktype("S");
            ...
            if(newlocktable.get(t).contains(Pair.with(i,"X"))){
                System.out.println("A transação {"+t.getName()+"} já possui o bloqueio do item {"+i.getName()+"}\n");
            }
            else{
    ...
    public void LX(Transaction t, DataItem i){
        ...
        //Se a transacao tem um bloqueio S e quer um bloqueio X e a fila do item esta vazia
        else if (newlocktable.get(t).contains(Pair.with(i,"S")) && i.getWaitqueue().isEmpty()){
            i.setLocktype("X");
            t.getBlockedItens().remove(Pair.with(i,"S"));
            t.getBlockedItens().add(Pair.with(i,"X"));
            newlocktable.get(t).clear();
            newlocktable.get(t).addAll(t.getBlockedItens());
        } //Troca o tipo do bloqueio de S para X
            ...
            i.setLocktype("X");
            t.getBlockedItens().add(Pair.with(i,"X"));
            newlocktable.put(t,t.getBlockedItens());
    ...
    public void U(Transaction t){
        ...
        Iterator<Pair<DataItem,String>> iterator = newlocktable.get(t).iterator();
        ...
        while(iterator.hasNext()){
            ...
            if(item_block.getValue0().getWaitqueue().isEmpty()) { //Se a wailist do DataItem estiver vazia
                ...
                transaction_block.getValue0().getBlockedItens().add(Pair.with(item_block.getValue0(),transaction_block.getValue1()));
                newlocktable.put(transaction_block.getValue0(),transaction_block.getValue0().getBlockedItens());
